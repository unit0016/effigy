...
/obj/item/clothing/head/deprivation_helmet
	...
	flags_inv = HIDEMASK|HIDEEARS|HIDEEYES|HIDEFACE|HIDEHAIR|HIDESNOUT|HIDEFACIALHAIR|HIDEHORNS
...
/obj/item/clothing/head/deprivation_helmet/proc/SwitchHelmet(button)
	...
	switch(user_client)
		if("speech")
			if(muzzle == TRUE)
				muzzle = FALSE
				play_lewd_sound(usr, 'sound/items/weapons/magout.ogg', 40, TRUE)
				...
				muzzle = TRUE
				play_lewd_sound(usr, 'sound/items/weapons/magin.ogg', 40, TRUE)
