...
/obj/item/clothing/glasses/hypno/equipped(mob/living/user, slot)
	...
	if(!(slot & ITEM_SLOT_EYES))
		return
	if(!(iscarbon(user) && user.client?.prefs?.read_preference(/datum/preference/toggle/erp_item_use)))
		return
	if(!codephrase)
...
/obj/item/clothing/glasses/hypno/process(seconds_per_tick)
	...
	if(!istype(wearer) || !SPT_PROB(1, seconds_per_tick))
		return
	if(!(wearer.client?.prefs?.read_preference(/datum/preference/toggle/erp_item_use))) // something is really fucked if they're wearing this at this point anyways
		return
	switch(rand(1, 2))
		if(1)
