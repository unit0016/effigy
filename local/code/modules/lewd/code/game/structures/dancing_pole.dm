...
/obj/structure/stripper_pole/proc/dance_animate(mob/living/user)
	if(user.loc != src.loc)
		return
	if(!QDELETED(src))
		animate(user, pixel_x = -6, pixel_z = 0, time = 10)
		sleep(2 SECONDS)
		user.dir = 4
	if(!QDELETED(src))
		animate(user, pixel_x = -6, pixel_z = 24, time = 10)
		sleep(1.2 SECONDS)
	...
	if(!QDELETED(src))
		animate(user, pixel_x = 6, pixel_z = 12, time = 5)
		user.dir = 8
		sleep(0.6 SECONDS)
	if(!QDELETED(src))
		animate(user, pixel_x = -6, pixel_z = 4, time = 5)
		user.dir = 4
		src.layer = 4 // move it back.
		sleep(0.6 SECONDS)
	if(!QDELETED(src))
		user.dir = 1
		animate(user, pixel_x = 0, pixel_z = 0, time = 3)
		sleep(0.6 SECONDS)
	if(!QDELETED(src))
