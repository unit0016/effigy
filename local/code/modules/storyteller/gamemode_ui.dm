...
/datum/controller/subsystem/gamemode/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("set_storyteller")
			...
			var/new_storyteller_name = input(usr, "Choose new game mode (circumvents voted one):", "Game Mode")  as null|anything in name_list
			if(!new_storyteller_name)
				return
			message_admins("[key_name_admin(usr)] has changed the game mode to [new_storyteller_name].")
