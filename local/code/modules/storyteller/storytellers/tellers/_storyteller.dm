...
/datum/storyteller/proc/find_and_buy_event_from_track(track)
	...
	if(SSgamemode.forced_next_events[track]) //Forced event by admin
		...
		if(player_pop < pop_required)
			message_admins("Game mode failed to pick an event for track of [track] due to insufficient population. (required: [pop_required] active pop for [track]. Current: [player_pop])")
			log_admin("Game mode failed to pick an event for track of [track] due to insufficient population. (required: [pop_required] active pop for [track]. Current: [player_pop])")
		...
		if(!length(valid_events))
			message_admins("Game mode failed to pick an event for track of [track].")
			log_admin("Game mode failed to pick an event for track of [track].")
			SSgamemode.event_track_points[track] *= TRACK_FAIL_POINT_PENALTY_MULTIPLIER
			return
		picked_event = pick_weight(valid_events)
		if(!picked_event)
			message_admins("WARNING: Game mode picked a null from event pool. Aborting event roll.")
			log_admin("WARNING: Game mode picked a null from event pool. Aborting event roll.")
			stack_trace("WARNING: Game mode [src] picked a null from event pool at track [track].")
			return
...
/datum/storyteller/proc/buy_event(datum/round_event_control/bought_event, track)
	...
	message_admins("Game mode purchased and triggered [bought_event] event, on [track] track, for [total_cost] cost.")
	log_admin("Game mode purchased and triggered [bought_event] event, on [track] track, for [total_cost] cost.")
	if(bought_event.roundstart)
