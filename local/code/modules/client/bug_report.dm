/client/verb/effigy_issue_report()
	...
	if(!effigy_player_id)
		to_chat(src, type = MESSAGE_TYPE_SYSTEM, html = alert_boxed_message(RED, "Failed to start a new Issue Report: Please create an Issue Report to report this issue."))
		to_chat(src, type = MESSAGE_TYPE_SYSTEM, html = alert_boxed_message(RED, "Alternatively, scream into the void. Both are equally effective options right now."))
		return

	title = tgui_input_text(src, message = "", title = "Issue Report - Title", max_length = 128, multiline = FALSE, encode = TRUE, timeout = 0)

	if(!title)
		to_chat(src, type = MESSAGE_TYPE_SYSTEM, html = alert_boxed_message(RED, "Failed to start a new Issue Report: You need to put a title for the submission!"))
		return
	...
	if(!content)
		to_chat(src, type = MESSAGE_TYPE_SYSTEM, html = alert_boxed_message(RED, "Failed to start a new Issue Report: You need to put content in the submission!"))
		return

	var/server_rev = copytext(GLOB.revdata.commit, 1, 8)
	var/round_real_time = REALTIMEOFDAY - SSticker.round_utc_start_time
	...
	to_chat(src, type = MESSAGE_TYPE_SYSTEM, html = alert_boxed_message(BLUE, "Issue Report: [event_id] created."))
