...
// In this codebase Subtle and subtler are functionally the same
...
/datum/emote/living/subtle/run_emote(mob/user, params, type_override = null)
	...
	if(SSdbcore.IsConnected() && is_banned_from(user, "emote"))
		...
		subtle_emote = tgui_input_text(user, "Choose an emote to display.", "Subtle Anti-Ghost" , null, MAX_MESSAGE_LEN, TRUE)
		if(!subtle_emote)
			return FALSE
		...
		for(var/mob/eye/camera/remote/ai_eye in in_view)
