/datum/job/ai/proc/effigy_ai_after_spawn(mob/living/silicon/ai/ai_spawn, client/player_client)
	...
	for(var/mob/living/silicon/robot/sync_target in GLOB.silicon_mobs)
		if(!(sync_target.z in SSmapping.levels_by_trait(ZTRAIT_STATION)) || (sync_target.z in SSmapping.levels_by_trait(ZTRAIT_ICE_RUINS_UNDERGROUND))) // Skip "other" cyborgs.
			continue
